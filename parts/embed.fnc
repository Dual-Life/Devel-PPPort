...
:# namespace clashes.
:#
:# To make a function visible everywhere on all platforms use the A flag to
:# make it part of Perl's API contract with XS developers, or the X flag to
:# indicate it is intended for core use only.  The E flag implies the X flag
:# and indicates it is for the use by core and Perl extensions.
:#
...
:#
:# and with the same meanings as the lines in this file.  However, since these
:# are macros, the arguments need not be legal C parameters.  To indicate this
:# to downstream software that inspects these lines, there are a few
:# conventions:
...
:#
:# Additionally, "XXXliteral string" (including the quotes) should be used to give
:# the argument type if it has to be a literal string constant, like
:#   =for apidoc Ama|SV*|newSVpvs|"string"
:#
:# autodoc uses these lines in conjunction with this file to construct
:# perlapi.pod.  For entries of the first form (with just the macro name),
:# there must be a corresponding entry in this file, and the purpose of the
:# line is to associate the pod accompanying it with the interface defined in
:# this file.  The second form is used to define the interface for the
:# documentation when there is no entry in this file, hence it will be for
:# something that isn't a non-static function: a macro of some kind, a
:# constant, or some other entity that it wishes to have documented.  (If there
:# is also an entry in this file it overrides the apidoc line, and autodoc.pl
:# will warn.) 'return_type' in these lines can be empty, unlike in this file.
:# The entries in this file that have corresponding '=for apidoc' entries
:# should have the flag 'd' set in this file.
:#
...
:#      (using a different implementation), also specify the 'M' (XXX or m?) flag.  The 'b'
...
:#	  add embed.h entry (unless overridden by the ?XXX 'm' flag)
:#
...
:#      is implied if A isn't also specified.  This is similar to the x flag,
:#      but it doesn't generate a warning that the function is experimental in
...
:#
:#   u  The macro's (it has to be a macro) parameters aren't C parameters, or
:#      the macro expands to something that isn't a symbol.
:#
:#	  For example, the expansion of aTHX_ includes a comma, so would have
:#         this flag; or some macros take preprocessor tokens, so would have
:#         this flag.  This flag is an indication to downstream tools, such as
:#         Devel::PPPort, that this requires special handling.
:#
...
: Only Perl_ form should be tested
...
hApd	|void	|save_aptr	|NN AV** aptr
hApd	|AV*	|save_ary	|NN GV* gv
...
hAbpDd	|void	|save_list	|NN SV** sarg|I32 maxsarg
...
pXE	|SV*	|sv_setsv_cow	|NULLOK SV* dstr|NN SV* sstr
#endif
